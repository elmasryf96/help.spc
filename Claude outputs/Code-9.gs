function doGet(e) {
  ...
  if (e.parameter && e.parameter.action === "queueCallDetail") {
    var queueCallDetail = getQueueCallDetail_(e.parameter);
    return ContentService.createTextOutput(JSON.stringify(queueCallDetail))
      .setMimeType(ContentService.MimeType.JSON);
  }
...
function editAgentStatusPoint_(ss, data) {
  ...
  // OldStatus اختياري - لو الأدمن حدده صراحة من دروب داون "From" في قايمة الـ Sessions، بنستخدمه زي ما هو
  // بدل ما نحاول نخمنه (شوف computedOldStatus تحت). فاضي = زي الأول بالظبط (التخمين التلقائي - المودال
  // القديم اللي بيفتح من الدوس على التايم لاين لسه بيشتغل زي ما هو، ده اختياري ومحدش مجبر يبعته)
  var explicitOldStatus = String(data.oldStatus || "").trim();
  ...
  // حالة أ: نفس الوقت بالظبط، الحالة (و/أو OldStatus لو اتبعت صراحة) بس اتغيرت -> تعديل مكانه في الشيت مباشرة
  if (matchedIdx !== -1 && originalTimestamp === newTimestamp) {
    sheet.getRange(rows[matchedIdx].rowNum, 4).setValue(newStatus);
    if (explicitOldStatus) {
      sheet.getRange(rows[matchedIdx].rowNum, 3).setValue(explicitOldStatus);
    }
  ...
  // الـ OldStatus بتاع الصف الجديد: لو الأدمن حدده صراحة (explicitOldStatus) بنستخدمه زي ما هو، وإلا
  // بنخمّنه زي الأول = آخر حالة كانت شغالة قبله زمنيًا لنفس الإيجنت (أو "Away" لو دي أول حركة ليه في السجل
  // كله) - مستخدمة بس في حساب "أول لوجين"/"آخر شيفت" (buildOneDayReport_)، مش في رسم الجلسات نفسها
  var computedOldStatus = explicitOldStatus;
  if (!computedOldStatus) {
    computedOldStatus = "Away";
  ...
  // لو الصف اللي اتحرك (نقل وقت - حالة ب) كان هو نقطة "أول لوجين" ليوم قديم (OldStatus="Away")، وبقى
  // دلوقتي في مكان/يوم تاني، لازم ننقل الدور ده لأقدم صف متبقي في اليوم القديم - نفس السبب بالظبط اللي في
  // deleteAgentStatusPoint_ / ensureDayLoginAnchor_
  if (matchedIdx !== -1 && rows[matchedIdx].oldStatus === "Away") {
    ensureDayLoginAnchor_(sheet, agentName, rows[matchedIdx].timestamp.substring(0, 10));
  }
  ...
  if (lastRow >= 2) {
    ...
    for (var i = 0; i < values.length; i++) {
      ...
      if (name === agentName && ts === originalTimestamp) {
        var deletedOldStatus = String(values[i][2]).trim();
        sheet.deleteRow(i + 2);

        // لو الصف اللي اتمسح كان هو نقطة "أول لوجين" لليوم ده (OldStatus="Away")، لازم ننقل الدور ده
        // لأقدم صف متبقي لنفس الإيجنت في نفس اليوم - وإلا buildOneDayReport_ مش هيلاقي أي صف
        // OldStatus="Away" لليوم ده، وهيرجع firstLogin=null حتى لو الإيجنت اشتغل طول اليوم فعلاً (بيبوظ
        // "No Show" غلط - ده اللي كان بيحصل قبل الإصلاح ده لو مسحت أول نقطة في اليوم من قايمة الـ Sessions)
        if (deletedOldStatus === "Away") {
          ensureDayLoginAnchor_(sheet, agentName, originalTimestamp.substring(0, 10));
        }
...
// ------------------------------------------------------------
// 🩹 بيتأكد إن يوم معين لإيجنت معين لسه ليه "نقطة أول لوجين" (صف OldStatus="Away") بعد أي حذف/نقل ممكن
// يكون شال النقطة دي منه - لو مفيش، بيرقّي أقدم صف متبقي لنفس الإيجنت في نفس اليوم عشان يبقى هو الأول
// لوجين بدالها. بينادَى من deleteAgentStatusPoint_ وeditAgentStatusPoint_ (حالة نقل الوقت) بس لما الصف
// المتأثر يكون هو نفسه كان شايل الدور ده أصلاً
// ------------------------------------------------------------
function ensureDayLoginAnchor_(sheet, agentName, dateStr) {
  var lastRow = sheet.getLastRow();
  if (lastRow < 2) return;
  var values = sheet.getRange(2, 1, lastRow - 1, 5).getValues();

  var hasAwayAnchor = false;
  var earliestIdx = -1;
  var earliestTs = null;
  for (var i = 0; i < values.length; i++) {
    var name = String(values[i][0]).trim();
    var ts = String(values[i][4]).replace(/^'/, "").trim();
    if (name !== agentName || ts.substring(0, 10) !== dateStr) continue;
    if (String(values[i][2]).trim() === "Away") { hasAwayAnchor = true; break; }
    if (earliestTs === null || ts < earliestTs) { earliestTs = ts; earliestIdx = i; }
  }

  if (!hasAwayAnchor && earliestIdx !== -1) {
    sheet.getRange(earliestIdx + 2, 3).setValue("Away");
  }
}
...
function buildOneDayReport_(allRows, dateStr) {
  ...
  // احتياطي: لو مفيش صف OldStatus="Away" لليوم ده (مثلاً لو نقطة "أول لوجين" الأصلية اتمسحت قبل إصلاح
  // ensureDayLoginAnchor_، أو أي سبب تاني)، اعتبر أقدم صف في اليوم هو أول لوجين برضو - أحسن بكتير من ما
  // نرجّع null ونخلي getAttendanceStatus في الفرونت يقلب "No Show" غلط حتى لو الإيجنت اشتغل طول اليوم فعلاً
  if (firstLogin === null && dayRows.length > 0) {
    firstLogin = dayRows[0].timestamp;
  }
  ...
  for (var j = 0; j < dayRows.length; j++) {
    ...
    // oldStatus بتتبعت للفرونت كمان (مكنتش قبل كده) - عشان قايمة الـ Sessions تقدر تعرض/تعدّل "From" صراحة
    // بدل ما السيرفر يخمّنه لوحده (شوف editAgentStatusPoint_ -> explicitOldStatus)
    sessions.push({ status: status, oldStatus: dayRows[j].oldStatus, start: start, end: end, durationSeconds: durationSec });
...
function getCallLogReport_(params) {
  ...
  var perAgentByDay = {}; // تاريخ -> اسم مجرد -> نفس الأرقام - مطلوبة لتصدير الإكسل (كل يوم لوحده)

  function emptyAgentStats_() {
    return { calls: 0, talkSeconds: 0, outboundAnsweredCount: 0, outboundUnansweredCount: 0, outboundTalkSeconds: 0 };
  }
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    if (!perAgent[agentPlain]) perAgent[agentPlain] = emptyAgentStats_();

    // نفس منطق استخراج التاريخ المستخدم في computeQueueSummaryByDay_ بالظبط
    var rawDate = row[0];
    var dateVal = (Object.prototype.toString.call(rawDate) === "[object Date]")
      ? Utilities.formatDate(rawDate, "Asia/Dubai", "yyyy-MM-dd")
      : String(rawDate).replace(/^'/, "").trim();

    if (!perAgentByDay[dateVal]) perAgentByDay[dateVal] = {};
    if (!perAgentByDay[dateVal][agentPlain]) perAgentByDay[dateVal][agentPlain] = emptyAgentStats_();
    var dayStats = perAgentByDay[dateVal][agentPlain];
    ...
    if (direction === "Outbound") {
      ...
      if (outboundResult === "Answered") {
        ...
        dayStats.outboundAnsweredCount += 1;
        // بنجمع مدة المكالمات الصادرة المردود عليها كمان - مطلوبة لحساب Occupancy
        // (نسبة وقت الاشتغال الفعلي على مكالمات من إجمالي وقت الـ Login)
        var outboundTalk = (Number(row[7]) || 0);
        perAgent[agentPlain].outboundTalkSeconds += outboundTalk;
        dayStats.outboundTalkSeconds += outboundTalk;
      } else {
        perAgent[agentPlain].outboundUnansweredCount += 1;
        dayStats.outboundUnansweredCount += 1;
      }
      continue; // مش بتدخل في حساب Calls Answered / AHT بتاع الكيو (الداخلة بس)
    }
    ...
    dayStats.calls += 1;
    dayStats.talkSeconds += talkSeconds;
  }

  function finalizeAgentStats_(agentName, stats) {
    return {
      agent: agentName,
      ...
      outboundTalkSeconds: Math.round(stats.outboundTalkSeconds * 10) / 10,
  ...
  for (var agentName in perAgent) {
    agents.push(finalizeAgentStats_(agentName, perAgent[agentName]));
  }

  // agentsByDay: { "2026-09-01": { "Ahmed": {agent, callsAnswered, ahtSeconds, ...}, ... }, ... }
  // مطلوبة لتصدير الإكسل عشان كل صف (إيجنت + يوم) ياخد أرقام المكالمات بتاعته هو في اليوم دا بالظبط
  var agentsByDay = {};
  for (var dv in perAgentByDay) {
    agentsByDay[dv] = {};
    for (var an in perAgentByDay[dv]) {
      agentsByDay[dv][an] = finalizeAgentStats_(an, perAgentByDay[dv][an]);
    }
  }
  ...
  if (params.name) {
    ...
    return {
      ...
      mode: params.mode || "day",
      agent: params.name,
      data: match || { agent: params.name, callsAnswered: 0, totalTalkSeconds: 0, ahtSeconds: 0, outboundAnsweredCount: 0, outboundUnansweredCount: 0, outboundTalkSeconds: 0, outboundCallsCount: 0 },
      queueSummary: queueSummary,
      queueSummaryByDay: queueSummaryByDay,
      queueSummaryByHour: queueSummaryByHour,
      agentsByDay: agentsByDay
    };
  }

  return { status: "success", mode: params.mode || "day", agents: agents, queueSummary: queueSummary, queueSummaryByDay: queueSummaryByDay, queueSummaryByHour: queueSummaryByHour, agentsByDay: agentsByDay };
}
...
// ------------------------------------------------------------
// 📞 نفس فكرة getCallLogDetail_ فوق، بس على مستوى الكيو كله (كل الإيجنتس)
// مش إيجنت بعينه - مطلوبة لمكالمات زي "Abandoned" اللي أصلاً معندهاش إيجنت
// اترد عليها فيرجع اسم واضح ليها في عمود Agent، فمفيش معنى نفلتر بالاسم.
// بترجع كل صفوف الفترة المطابقة للـ direction/result المطلوبين، بغض النظر
// مين الإيجنت (أو لو العمود فاضي/غير محدد).
// استخدام:
//   ?action=queueCallDetail&mode=day&date=2026-09-07&direction=Inbound&result=Abandoned
//   ?action=queueCallDetail&mode=range&start=...&end=...&direction=Inbound&result=Abandoned
// ------------------------------------------------------------
function getQueueCallDetail_(params) {
  var dateList = buildDateList_(params.mode || "day", params);
  if (!dateList) return { status: "error", message: "Invalid date parameters" };
  ...
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var sheet = ss.getSheetByName("Call Log");
  if (!sheet) return { status: "success", calls: [] };

  var wantDirection = String(params.direction || "Inbound").trim();
  var wantResult = params.result ? String(params.result).trim() : null; // مفيش result = أي نتيجة

  var directionColIdx = CALL_LOG_HEADERS.indexOf("Direction");
  var queueColIdx = CALL_LOG_HEADERS.indexOf("Queue");
  var data = readCallLogRowsForDates_(sheet, dateSet); // بس صفوف التواريخ المطلوبة، من غير هيدر
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    calls.push({
      ...
      time: timeVal,
      customerNumber: String(row[2]).replace(/^'/, "").trim(),
      queue: String(row[queueColIdx] || "").replace(/^'/, "").trim(),
      result: result,
      waitSeconds: Number(row[6]) || 0
    });
  }

  // الأحدث أولاً - نفس أسلوب getCallLogDetail_
  calls.sort(function (a, b) {
    var da = a.date + " " + a.time, db = b.date + " " + b.time;
    return da < db ? 1 : (da > db ? -1 : 0);
  });

  return { status: "success", direction: wantDirection, result: wantResult, calls: calls };
}
