...
function getTodayStatusLog_() {
  ...
  var data = sheet.getDataRange().getValues(); // Name | Number | Old Status | New Status | Timestamp
  var rows = [];
  for (var i = 1; i < data.length; i++) {
    var ts = String(data[i][4]).replace(/^'/, "").trim();
    if (ts.indexOf(todayStr) !== 0) continue;
    rows.push({
      name: String(data[i][0]).trim(),
      number: String(data[i][1]).trim(),
      oldStatus: String(data[i][2]).trim(),
      newStatus: String(data[i][3]).trim(),
      timestamp: ts
    });
  }
...
function getAgentStatusReport_(params) {
  ...
  for (var i = 1; i < data.length; i++) {
    ...
    var ts = String(data[i][4]).replace(/^'/, "").trim(); // "yyyy-MM-dd HH:mm:ss"
    allRows.push({
      oldStatus: String(data[i][2]).trim(),
      newStatus: String(data[i][3]).trim(),
      timestamp: ts
  ...
  return {
    ...
    trackingStartDate: getEarliestLogDate_(data)
  };
}
...
// بيرجع أقدم تاريخ فيه بيانات في شيت AgentStatusLog كله (مش لإيجنت معين) بصيغة yyyy-MM-dd، أو null لو الشيت فاضي
function getEarliestLogDate_(data) {
  var earliest = null;
  for (var i = 1; i < data.length; i++) {
    var ts = String(data[i][4]).replace(/^'/, "").trim();
    if (!ts) continue;
...
function getAllAgentsLoginTotals_(params) {
  ...
  for (var i = 1; i < data.length; i++) {
    var name = String(data[i][0]).trim();
    if (name === "") continue;
    var number = String(data[i][1]).trim();
    var ts = String(data[i][4]).replace(/^'/, "").trim();
  ...
  return { status: "success", mode: mode, agents: agents, trackingStartDate: getEarliestLogDate_(data) };
}
function doPost(e) {
...
// ------------------------------------------------------------
// ⚡ محسّنة: بتاخد data/directionColIdx جاهزين (بدل ما تقرا الشيت تاني من الأول)
// عشان getCallLogReport_ يقرا شيت Call Log مرة واحدة بس ويشاركها مع الدوال
// دي كلها - كان بيتقرا 4 مرات منفصلة لكل طلب واحد، وده بقى مكلف بعد ما
// الشيت اتضاعف حجمه في الباكفيل (كان بيسبب بطء/تايم آوت متقطع في صفحة
// Live Agent Status)
...
// ⚡ محسّنة: بتاخد data/directionColIdx جاهزين برضو (شوف تعليق computeQueueSummary_)
