...
# ------------------------------------------------------------
# 🔍 تشخيص مؤقت: كل صفوف مكالمة معينة (كيو + كل الإيجنتس اللي 3CX حاول يوصلهم بيها)
# عن طريق رقم العميل، في آخر N دقيقة - قبل ما نبني ميزة "مين عمل Reject للمكالمة"،
# محتاجين نشوف شكل الـ Reason/Answered الخام لصف الإيجنت اللي عمل Reject فعليًا،
# عشان نتأكد لو 3CX بيفرّق أصلاً بين "رفض المكالمة يدوي" و"معملش حاجة لحد ما فات وقتها"
# استخدام: /debug/3cx-call-detail?secret=...&number=0567542880&minutes=30
# ------------------------------------------------------------

@app.get("/debug/3cx-call-detail")
async def debug_3cx_call_detail(secret: str = "", number: str = "", minutes: int = 30):
    if secret != os.environ.get("SYNC_SECRET", ""):
        return {"status": "error", "message": "Unauthorized"}
    if not number:
        return {"status": "error", "message": "Missing 'number' param - رقم العميل اللي عايزين نتتبع مكالمته"}

    period_to = datetime.utcnow()
    period_from = period_to - timedelta(minutes=minutes)
    ...
    async with httpx.AsyncClient(timeout=20) as client:
        ...
        try:
            for _ in range(10):
                resp = await client.get(build_url(skip), headers=headers)
                data = resp.json()
                rows = data.get("value", [])
                if not rows:
                    break
            ...
            return {"error": str(e), "rows_fetched_so_far": len(all_rows)}

        # بنلاقي كل الـ MainCallHistoryId اللي فيها الرقم ده (كعميل)، وبعدين بنرجّع
        # كل صفوفها (كيو + كل محاولة مع كل إيجنت) - مش بس صف الكيو النهائي
        matching_ids = set()
        for row in all_rows:
            src = str(row.get("SourceCallerId") or row.get("SourceDn") or "")
            dst = str(row.get("DestinationCallerId") or row.get("DestinationDn") or "")
            if number and (number in src or number in dst):
                matching_ids.add(row.get("MainCallHistoryId"))

        matched_rows = [r for r in all_rows if r.get("MainCallHistoryId") in matching_ids]
        matched_rows.sort(key=lambda r: r.get("StartTime") or "")

        return {
            "status": "success",
            "total_rows_scanned": len(all_rows),
            "matched_calls": len(matching_ids),
            "rows": matched_rows,
        }
